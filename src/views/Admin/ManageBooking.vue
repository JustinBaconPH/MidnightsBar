...
const archiveBooking = (id) => {
  ...
  axios.post('http://localhost/GRP5_MIDNIGHTS/backend/bookapi.php', data)
    .then(() => {
      fetchBook();
    })
    .catch((error) => {
...
const fetchBook = () => {
  const url = showArchived.value ? 'http://localhost/GRP5_MIDNIGHTS/backend/bookapi.php?action=get_all_deleted' : 'http://localhost/GRP5_MIDNIGHTS/backend/bookapi.php?action=get_all_notdeleted';
  axios.get(url)
    .then((response) => {
